...
class Atten( nn.Module ):
    ...
    def forward( self, V, h_t, s_t ):
        ...
        c_t_spatial = torch.bmm( alpha_t, V ).squeeze( 2 )  # B * seq * 2048
        c_t_channel = alpha0.unsqueeze(2) * V.unsqueeze(1)
        c_t_channel = torch.mean(c_t_channel,3) # B * seq * 49
        #c_t_s_norm = c_t_spatial.norm(p=2,dim=2) + 1e-8
        #c_t_spatial = c_t_spatial / c_t_s_norm.view(c_t_s_norm.size(0), c_t_s_norm.size(1),-1)
        #c_t_c_norm = c_t_channel.norm(p=2,dim=2) + 1e-8
        #c_t_channel = c_t_channel / c_t_c_norm.view(c_t_c_norm.size(0), c_t_c_norm.size(1),-1)
        c_t_spatial = F.relu(c_t_spatial)
        c_t_channel = F.relu(c_t_channel)
        ...
        c_hat_t = s_gate*(spatial_info + channel_info)
        # c_hat_t = s_t + c_t + h_t
