...
class Atten( nn.Module ):
    ...
    def forward( self, V, h_t, s_t ):
        ...
        cxt_ = feature_vec+ self.affine_g2( self.dropout( h_t ) ).unsqueeze( 2 )
        ...
        # W_v * V + W_g * h_t * 1^T
        content_v = self.affine_v( self.dropout( weightedfeature ) ) \
...
class Encoder2Decoder( nn.Module ):
    ...
    def mysampler( self, images, max_len=20, beam_size=3 ):
        """
        Samples captions for given image features (Greedy search).
        """
...
#                     state_list[i][1][j]=newstate_list[whichbeam][1][j]
                    ...
                    cur_prob = np.log(logits_list[j][idx])
