...
class CaptionGenerator(object):
    ...
    def sca_layer(self, features, h, reuse=False):
        with tf.variable_scope('sca_layer', reuse=reuse):
            ...
            w1_2 = tf.get_variable('w1_2', [self.H, self.M], initializer=self.weight_initializer)
            pstate_1 = tf.expand_dims(tf.matmul(h, w1_2),1)
            pctx1 = tf.nn.relu(pctx_1+pstate_1)
            w1_3 = tf.get_variable('w1_3', [self.M, 1], initializer=self.weight_initializer)
            b1_2 = tf.get_variable('b1_2', [1], initializer=self.const_initializer)
            alpha1_ = tf.reshape(tf.tensordot(pctx1,w1_3,[[2],[0]])+b1_2, [-1, self.D])
            alpha1 = tf.nn.softmax(alpha1_)  ## N * 512
            ...
            pctx2 = tf.nn.relu(pctx_2+pstate_2)
            w2_3 = tf.get_variable('w2_3', [self.M, 1], initializer=self.weight_initializer)
            #b2_2 = tf.get_variable('b2_2', [1], initializer=self.const_initializer)
            alpha2_ = tf.reshape(tf.tensordot(pctx2,w2_3,[[2],[0]]), [-1, 49])
            alpha2 = tf.nn.softmax(alpha2_)  ### N* 49
            alpha1_reshape = tf.expand_dims(alpha1,1)
            alpha2_reshape = tf.expand_dims(alpha2,2)
            context = tf.multiply(tf.multiply(features,alpha1_reshape),alpha2_reshape)
            context2 = tf.reshape(tf.reduce_sum(tf.multiply(features,alpha2_reshape),1),[-1,self.D], name='context')  # 512
            #contextnorm = tf.sqrt(tf.reshape(tf.reduce_sum(tf.square(context2),1),[-1,1]))
            #contextnorm = contextnorm+ tf.constant([1e-8])
            #context_normal = tf.divide(context2,contextnorm)
            ...
            #contextnorm2 = contextnorm2+ tf.constant([1e-8])
            #context_normal2 = tf.divide(context3,contextnorm2)       
            #return context_normal,context_normal2, alpha2_, alpha1_, w1_2
            return context2,context3, alpha2_, alpha1_ , w1_2, w2_3
    ...
    def mydecode_lstm2(self, x, sentinal, h, context,context2,alpha,alpha2, wh,wh2, dropout=False, reuse=False):
        with tf.variable_scope('mylogits', reuse=reuse):
            ...
            st = tf.matmul(h,ws)+bs
            sentinal_vec = tf.nn.relu(st+ tf.matmul(h,wh))
            #wss = tf.get_variable('wss', [self.D, 1], initializer=self.weight_initializer)
            sentinal_z = tf.matmul(tf.nn.dropout(sentinal_vec,0.5),wh2)
            alpha = tf.reshape(tf.reduce_mean(alpha,1),[-1,1])
           ...
           # ctx1b = tf.get_variable('ctx1b', [self.M], initializer=self.const_initializer)
            ...
            #wsout = tf.get_variable('wsout', [self.H, self.D], initializer=self.weight_initializer)
            #bsout = tf.get_variable('bsout', [self.D], initializer=self.const_initializer)

            #if dropout:
            #    h = tf.nn.dropout(h, 0.5)
            #h_logits = tf.matmul(h, w_h) + b_h
            h_logits = tf.nn.dropout(h,0.5)
            #h_logits = 0
            #h_logits = tf.nn.dropout(h,0.5)
            ...
            #h_logits =tf.concat([(context)*(1-betat),betat*sentinal,h_logits],1)
            h_logits +=tf.nn.dropout(context,0.5)*(1-betat)+betat*tf.nn.dropout(sentinal,0.5)
            #h_logits +=tf.nn.dropout(context,0.5)*(1-betat)+betat*tf.nn.dropout(x,0.5)
            #h_logits +=tf.nn.dropout(context,0.5)*(1-betat)+*tf.nn.dropout(sentinal,0.5)
            #h_logits += tf.nn.dropout(context,0.5)+tf.nn.dropout(sentinal,0.5)
           # if dropout:
            #    h_logits = tf.nn.dropout(h_logits, 0.5)
            out_logits = tf.matmul(h_logits, w_out) + b_out
            return out_logits
    ...
    def _2featuresemantic(self, poolfeatures, prevh, x, reuse):
        with tf.variable_scope('featuresemantic', reuse=reuse):
            convw = tf.truncated_normal([7,7,512,1024],stddev=0.1)
            features = tf.reshape(poolfeatures,[-1,7,7,512])
            outconv = tf.reshape(tf.nn.relu(tf.nn.conv2d(features,convw, strides=[1, 1, 1, 1], padding='VALID')),[-1,1024])
            w = tf.get_variable('w', [1024, self.D], initializer=self.weight_initializer)
            b = tf.get_variable('b', [self.D], initializer=self.const_initializer)
            # meanfeature = tf.reshape(tf.reduce_mean(poolfeatures, axis=1),[-1,self.D])
            fsemantic = tf.matmul(outconv,w)+b
            # hw = tf.get_variable('hw', [self.H, 1], initializer=self.weight_initializer)
            # xw = tf.get_variable('xw', [self.M, 1], initializer=self.weight_initializer)
            # gate = tf.nn.sigmoid(tf.matmul(prevh, tf.nn.dropout(hw,0.5))+tf.matmul(x,tf.nn.dropout(xw,0.5)))
            # out = tf.multiply(fsemantic,gate)
            #out = tf.nn.relu(fsemantic)
            return tf.nn.dropout(fsemantic,0.5)

    def _sentinal(self, x, prevh, c,wh, reuse):
        with tf.variable_scope('sential', reuse=reuse):
            ...
            gt = tf.nn.sigmoid(tf.matmul(x,wx)+bx+tf.matmul(prevh,wh))
            #out = tf.multiply(gt,tf.nn.tanh(c))
            out = tf.multiply(gt,tf.nn.tanh(c))
            #w = tf.get_variable('w', [self.M, self.H], initializer=self.weight_initializer)
    ...
    def build_model(self):
        ...
        features = self._pooling(features)
        ...
        for t in range(self.T):
            ...
            precontext = self._featuresemantic(features, h, x[:,t,:], reuse=(t!=0))  # N * 512
            prevh = h
            ...
            context, context2, alpha, alpha2,wh, wh2 = self.sca_layer(features, c, reuse=(t!=0))
            sentinal = self._sentinal(x[:,t,:], prevh, c, wh, reuse=(t!=0))
            # N * 512 
            if self.selector:
                context, beta = self._selector(context, h, reuse=(t!=0))
            alpha_list.append(alpha)
            alpha2_list.append(alpha2)
            logits = self.mydecode_lstm2(x[:,t,:],sentinal, h, context,context2,alpha, alpha2, wh,wh2, dropout=self.dropout, reuse=(t!=0))
	...
	alpha2_list = []
        ...
        for t in range(max_len):
            ...
            precontext = self._featuresemantic(features, h, x, reuse=(t!=0))  # N * 512
            prevh = h
            #precontext, prealpha = self._attention_layer(features,features_proj, h, reuse=(t!=0))
            ...
            context, context2, alpha, alpha2,wh,wh2 = self.sca_layer(features, c, reuse=(t!=0))
            sentinal = self._sentinal(x, prevh, c,wh, reuse=(t!=0))
            if self.selector:
                context, beta = self._selector(context, h, reuse=(t!=0))
                beta_list.append(beta)
            alpha_list.append(alpha)
	    alpha2_list.append(alpha2)
            logits = self.mydecode_lstm2(x,sentinal, h, context,context2,alpha,alpha2, wh,wh2, reuse=(t!=0))
