...
class CaptionGenerator(object):
    ...
    def mydecode_lstm(self, x, h, context,context2, dropout=False, reuse=False):
        with tf.variable_scope('logits', reuse=reuse):
            ...
            #pickh = tf.get_variable('ph', [self.H, 1], initializer=self.weight_initializer)
            #pickV = tf.get_variable('pV', [self.D, 1], initializer=self.weight_initializer)
            #pickV2 = tf.get_variable('pV2', [196,1], initializer=self.weight_initializer)
            ...
            h_logits += contextvec + contextvec2 +x
            #h_logits = tf.nn.tanh(h_logits)
            if dropout:
                h_logits = tf.nn.dropout(h_logits, 0.5)
            out_logits = tf.matmul(h_logits, w_out) + b_out
