...
class CaptionGenerator(object):
    ...
    def mydecode_lstm(self, x, h, context,context2, dropout=False, reuse=False):
        with tf.variable_scope('logits', reuse=reuse):
            ...
            xvec = tf.matmul(x,wx2)
            
            pickx = tf.get_variable('px', [self.M, 1], initializer=self.weight_initializer)
            pickh = tf.get_variable('px', [self.H, 1], initializer=self.weight_initializer)
            pickV = tf.get_variable('pV', [self.D, 1], initializer=self.weight_initializer)
            pickV2 = tf.get_variable('pV2', [49,1], initializer=self.weight_initializer)
            preb = tf.nn.sigmoid(tf.matmul(x,pickx)+tf.matmul(h,pickh)+tf.matmul(context,pickV), tf.matmul(context2,pickV2)) 
            #if self.prev2out:
            #    h_logits += x
            #h_logits = tf.concat([h_logits, contextvec, contextvec2, xvec],axis=1)
            h_logits += preb*(contextvec + contextvec2) +xvec*(1-preb)
            #h_logits = tf.nn.tanh(h_logits)
            if dropout:
                h_logits = tf.nn.dropout(h_logits, 0.5)
            out_logits = tf.matmul(h_logits, w_out) + b_out
