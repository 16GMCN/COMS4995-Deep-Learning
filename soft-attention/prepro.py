...
def _process_test_caption_data(caption_file, image_dir, max_length):
    ...
    data = []
    for i in range(len(caption_data['images'])):
        
        t = os.path.join(image_dir, str(caption_data['images'][i]['file_name']))
        data += [t]
    data = pd.DataFrame({'file_name':data})
    return data
...
def main():
    ...
    test_dataset = _process_test_caption_data(caption_file='data/annotations/image_info_test2014.json',
                                        image_dir='image/test2014_resized/',
                                        max_length=max_length)
    ...
    #save_pickle(train_dataset.sample(5000), 'data/train/train.annotations.pkl')
    ...
    for split in ['train', 'val', 'test']:
        ...
        if split == 'test':
            file_names = _build_file_names_test(annotations)
    ...
    with tf.Session() as sess:
        ...
        for split in ['train', 'val', 'test']:
            ...
            save_path = './data/%s/%s.5000features.hkl' % (split, split)
